...
- **原生联网搜索**（v0.3 新增）：设置里一键开启，使用提供商 API 侧的官方搜索工具——Anthropic（服务端 web_search，按次计费）、Kimi（内建 $web_search）、智谱 GLM（web_search 工具）；搜索由提供商执行，本项目不经手任何第三方搜索服务。其他提供商暂无官方搜索能力，可等后续第三方搜索 API 接入
...
- [ ] 第三方搜索 API 接入（补齐 DeepSeek / 豆包等无原生搜索的提供商）
